...
shannon.p2.x2y.effTE = matrix(0, nrow = 51, ncol = M-1)
rownames(shannon.p2.x2y.effTE) = c("USA",unique(Google.mob$sub_region_1)[-c(1,10)])
colnames(shannon.p2.x2y.effTE) = colnames(AL)[-1]

shannon.p2.y2x.effTE = matrix(0, nrow = 51, ncol = M-1)
rownames(shannon.p2.y2x.effTE) = c("USA",unique(Google.mob$sub_region_1)[-c(1,10)])
colnames(shannon.p2.y2x.effTE) = colnames(AL)[-1]

shannon.p2.x2y.pvalue = matrix(0, nrow = 51, ncol = M-1)
rownames(shannon.p2.x2y.pvalue) = c("USA",unique(Google.mob$sub_region_1)[-c(1,10)])
colnames(shannon.p2.x2y.pvalue) = colnames(AL)[-1]

shannon.p2.y2x.pvalue.pvalue = matrix(0, nrow = 51, ncol = M-1)
rownames(shannon.p2.y2x.pvalue.pvalue) = c("USA",unique(Google.mob$sub_region_1)[-c(1,10)])
colnames(shannon.p2.y2x.pvalue.pvalue) = colnames(AL)[-1]
    ...
    for (j in 2:M) {
        ...
        if (i == 1){
            if (j %in% c(3,9,10,12)){
                ...
                shannon.p2.x2y.effTE[i,j-1] = model$coef[1,2]
                shannon.p2.y2x.effTE[i,j-1] = model$coef[2,2]
                shannon.p2.x2y.pvalue[i,j-1] = model$coef[1,4]
                shannon.p2.y2x.pvalue.pvalue[i,j-1] = model$coef[2,4]
            } else {
